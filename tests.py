...
def test():
    ...
    scenario.h4("Go to lap 2")
    scenario += oracle.inc_epoch().run()
    scenario += oracle.inc_epoch().run()
    scenario += c1.next_lap().run(source=owner, now=now.add_minutes(1))
    scenario.h4("Go to lap 3")
    scenario += oracle.inc_epoch().run()
    scenario += oracle.inc_epoch().run()
    scenario += c1.next_lap().run(source=owner, now=now.add_minutes(2))
    scenario.h4("Go to lap 4")
    scenario += oracle.inc_epoch().run()
    scenario += oracle.inc_epoch().run()
    scenario += c1.next_lap().run(source=owner, now=now.add_minutes(3))
